...
public:
    ...
    {
        const char* host = std::getenv("ELEVATOR_SIM_CAN_HOST");
        const char* portText = std::getenv("ELEVATOR_SIM_CAN_PORT");
        if (host == nullptr || portText == nullptr)
        {
            return ecOperationStatus::InvalidArgument;
        }
        const long port = std::strtol(portText, nullptr, 10);
...
private:
    void run(const std::stop_token stopToken) noexcept
    {
        try
        {
            ...
            {
                socketHandle = connectLoopback(host_, port_);
